...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Allow owner to make any transition (override restrictions)
        if ($_SESSION['role'] !== 'owner') {
            // Check if the transition is valid
            if (!isset($valid_transitions[$batch['status']]) || 
                !in_array($status, $valid_transitions[$batch['status']])) {
                throw new Exception("Invalid status transition from {$batch['status']} to {$status}.");
            }
        }
        
        // Update batch status
        $update_query = "UPDATE manufacturing_batches 
                        SET status = ?, 
                            status_changed_by = ?,
        ...
        $update_stmt->bindParam(2, $updated_by);
        $update_stmt->bindParam(3, $notes);
        $update_stmt->bindParam(4, $batch_id);
        $update_stmt->execute();
