...
// Get ACTUAL material costs (only for completed batches)
// Get ACTUAL material costs (only for completed batches)
// Get ACTUAL material costs (using quantity_required if quantity_used is zero)
$material_cost_query = "
    SELECT COALESCE(SUM(
    ...
    AND mu.quantity_required > 0
";
$stmt = $db->prepare($material_cost_query);
$stmt->execute();
$actual_material_cost = $stmt->fetchColumn();

// Get ACTUAL manufacturing costs (only for completed batches)
$manufacturing_cost_query = "SELECT COALESCE(SUM(amount), 0) 
                             FROM manufacturing_costs
                             WHERE batch_id IN (SELECT id FROM manufacturing_batches WHERE status = 'completed')";
$stmt = $db->prepare($manufacturing_cost_query);
$stmt->execute();
$actual_manufacturing_cost = $stmt->fetchColumn();

// Calculate TRUE profit
$total_actual_cost = $actual_material_cost + $actual_manufacturing_cost;
$profit = $financial['total_sales'] - $total_actual_cost;
$profit_margin = $financial['total_sales'] > 0 ? ($profit / $financial['total_sales'] * 100) : 0;
...
// Calculate cash on hand (total investment minus spent amounts)
$cash_on_hand = $financial['total_investment'] - $financial['total_purchases'] - $financial['total_manufacturing_costs'];
...
<div class="dashboard-stats">
    <div class="stat-card">
        <div class="stat-value"><?php echo number_format($actual_material_cost, 2); ?></div>
        <div class="stat-label">Used Material Cost</div>
    </div>
    <div class="stat-card">
        <div class="stat-value"><?php echo number_format($total_actual_cost, 2); ?></div>
        <div class="stat-label">Total Cost (used material + manufacturing)</div>
    </div>
